...
use crate::interface::state::{AppMode, AppState, WorkerStatus};
use ratatui::{
    prelude::*,
...
fn draw_cluster_3d(f: &mut Frame, app: &AppState, area: Rect, cluster: &Cluster, title: &str) {
    ...
    let mut render_atoms: Vec<(f64, f64, f64, Color, f64)> = cluster
        .atoms
        .iter()
        .map(|a| {
    ...
    if render_atoms
        .iter()
        .any(|(x, y, z, _, _)| x.is_nan() || y.is_nan() || z.is_nan())
    {
        f.render_widget(
            Paragraph::new("Error: NaN Coordinates").style(Style::default().fg(COL_FAIL)),
            inner_area,
        );
        return;
    }

    let max_coord = render_atoms
        .iter()
    ...
    let rot_status = if app.viewport.auto_rotate {
        "Auto-Rot: ON"
    } else {
        "Auto-Rot: OFF"
    };
    let overlay = Paragraph::new(rot_status).style(
        Style::default()
            .fg(Color::DarkGray)
            .add_modifier(Modifier::ITALIC),
    );
    let overlay_area = Rect {
        x: inner_area.x + inner_area.width.saturating_sub(14),
        y: inner_area.y,
        width: 14,
        height: 1,
    };
...
fn draw_stats(f: &mut Frame, app: &AppState, area: Rect) {
    ...
    let text = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                format!("{:.5} eV", app.telemetry.global_min_energy),
                Style::default()
                    .fg(COL_SUCCESS)
                    .add_modifier(Modifier::BOLD),
            ),
...
fn draw_hall_of_fame(f: &mut Frame, app: &mut AppState, area: Rect) {
    ...
    let rows = app.hall_of_fame.iter().enumerate().map(|(i, c)| {
        ...
        let id_disp = if id_short.len() > 8 {
            &id_short[..8]
        } else {
            &id_short
        };
        let hash_disp = c.hash_key.as_deref().unwrap_or("-");
        let hash_short = if hash_disp.len() > 12 {
            &hash_disp[..12]
        } else {
            hash_disp
        };
    ...
    .block(
        Block::default()
            .borders(Borders::ALL)
            .title(format!(" Hall of Fame ({}) ", app.hall_of_fame.len())),
    )
