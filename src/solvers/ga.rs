...
use crate::engine::operators::{crossover_cut_splice, Mutator};
...
impl GeneticAlgorithm {
    ...
    pub fn solve(&self, tx: Sender<SolverEvent>) {
        ...
        if population.is_empty() {
            let _ = tx.send(SolverEvent::Log(
                "CRITICAL: Failed to generate valid initial population.".to_string(),
            ));
        ...
        let mut last_global_best_e = population
            .first()
            .and_then(|c| c.energy)
            .unwrap_or(f64::MAX);
        ...
        for gen in 1..=self.params.max_steps {
            ...
            if !population.is_empty() {
                while next_gen.len() < self.params.population_size {
                    ...
                    if rng.gen::<f64>() < current_mutation_rate {
                        ...
                        let mut mutator = Mutator::new().rotate(0.5).rattle(rattle_mag).swap(1);
            ...
            if extinction_cooldown > 0 {
                ...
                if catastrophic_stagnation || premature_convergence {
                    let reason = if catastrophic_stagnation {
                        "Stagnation 50+"
                    } else {
                        "Low Diversity"
                    };
                    let _ = tx.send(SolverEvent::Log(format!(
                        "Mass Extinction ({}) -> Reseeding",
                        reason
                    )));
                    ...
                    while population.len() < self.params.population_size
                        && attempts < self.params.population_size * 100
                    {
                        attempts += 1;
                    ...
                    if current_mutation_rate < 0.5 {
                        let _ = tx.send(SolverEvent::Log(
                            "Stagnation (20+) -> Hyper-Mutation".to_string(),
                        ));
            ...
            // F. Telemetry
            let valid_clusters: Vec<&Cluster> =
                population.iter().filter(|c| c.energy.is_some()).collect();
    ...
    fn generate_initial_population(&self) -> Vec<Cluster> {
        ...
        for _ in 0..attempts {
            if pop.len() >= self.params.population_size {
                break;
            }
    ...
    fn evaluate_batch(&self, pop: &mut [Cluster]) -> usize {
        ...
        pop.par_iter_mut()
            ...
            .for_each(|cluster| match eval_ref.evaluate(cluster) {
                Ok(res) => {
    ...
    fn rank_population(&self, pop: &mut Vec<Cluster>) {
        pop.sort_by(|a, b| match (a.energy, b.energy) {
    ...
    fn deduplicate_population(&self, pop: Vec<Cluster>) -> (Vec<Cluster>, f64) {
        ...
        for c in pop {
            if c.status == ClusterStatus::Discarded || c.energy.is_none() {
                continue;
            }
