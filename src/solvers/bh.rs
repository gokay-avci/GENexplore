    ...
    pub fn solve(&self, mut current: Cluster, tx: Sender<SolverEvent>) {
        ...
        if current.energy.is_none() {
            let _ = tx.send(SolverEvent::Log(
                "Relaxing initial structure...".to_string(),
            ));
        ...
        let rate = if duration > 0.0 {
            self.params.max_steps as f64 / duration
        } else {
            0.0
        };

        let _ = tx.send(SolverEvent::Log(format!(
            "BH Finished. Acceptance: {}/{}",
            accepted_count, self.params.max_steps
        )));
