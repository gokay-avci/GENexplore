...
impl GulpEvaluator {
    ...
    fn run_process(&self, input_data: &str) -> Result<String> {
        ...
        let output = child
            .wait_with_output()
            .context("Failed to read GULP output")?;
    ...
    fn parse_geometry(&self, output: &str, original: &Cluster) -> Result<Cluster> {
        ...
        let start =
            start_idx.ok_or_else(|| anyhow!("No final coordinates found in GULP output"))?;
        ...
        for line in lines.into_iter().skip(start) {
            ...
            if parts[2].to_lowercase().starts_with('s') {
                continue;
            }
        ...
        if count != expected_atoms {
            bail!(
                "GULP atom count mismatch: expected {}, got {}. Geometry update aborted.",
                expected_atoms,
                count
            );
        }
